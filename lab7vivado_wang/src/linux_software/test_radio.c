...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float time_spent = (stop_time - start_time) * 8e-9;  // 8 ns per clock cycle, converted to seconds
    
    // Calculate the data transferred
    unsigned int bytes_transferred = 8192;  // 2048 words * 4 bytes per word = bytes transfered
    
    // Calculate throughput in MB/s
    float throughput = (bytes_transferred / time_spent * 1e-6);  // Convert bytes to MB and divide by time
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 7 Binhua Wang - Custom Peripheral Demonstration\n\r");
